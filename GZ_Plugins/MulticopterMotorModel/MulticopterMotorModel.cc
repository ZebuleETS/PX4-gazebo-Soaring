...
class gz::sim::systems::MulticopterMotorModelPrivate
{
  ...
  /// \brief Publisher for thrust data.
  public: transport::Node::Publisher thrustPub;

  /// \brief Publisher for motor efficiency data.
  public: transport::Node::Publisher efficiencyPub;

  /// \brief Publisher for electrical power data.
  public: transport::Node::Publisher electricalPowerPub;

  /// \brief Compute motor efficiency based on load and RPM.
  /// \param[in] _loadFactor Normalized load (0-1): thrust / max_thrust
  /// \param[in] _rpmFactor Normalized RPM (0-1): omega / max_omega
  /// \return Motor efficiency (0.65-0.90)
  public: double ComputeMotorEfficiency(double _loadFactor, double _rpmFactor)
  {
    // Efficacité maximale à charge moyenne (60-80%)
    double baseEfficiency;
    if (_loadFactor >= 0.6 && _loadFactor <= 0.8)
    {
      baseEfficiency = 0.85;  // Efficacité maximale
    }
    else if (_loadFactor < 0.3)
    {
      baseEfficiency = 0.70;  // Faible charge - efficacité réduite
    }
    else if (_loadFactor > 0.9)
    {
      baseEfficiency = 0.75;  // Charge très élevée - efficacité réduite
    }
    else
    {
      baseEfficiency = 0.80;  // Charge moyenne
    }

    // Ajustement avec RPM (meilleure efficacité à RPM optimal ~70% du max)
    double optimalRpmFactor = 0.7;
    double rpmPenalty = std::abs(_rpmFactor - optimalRpmFactor) * 0.1;

    double efficiency = baseEfficiency - rpmPenalty;
    // Limiter entre 65% et 90%
    return std::max(0.65, std::min(0.90, efficiency));
  }
};
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  // Create publisher for thrust (per-model topic for multi-UAV support)
  // Publish to robotNamespace + "/thrust" so each UAV has its own thrust topic
  std::string thrustTopic = transport::TopicUtils::AsValidTopic(
      this->dataPtr->robotNamespace + "/thrust");
  if (thrustTopic.empty())
  {
    gzwarn << "Failed to create valid thrust topic from ["
           << this->dataPtr->robotNamespace << "/thrust]" << std::endl;
  }
  else
  {
    this->dataPtr->thrustPub = this->dataPtr->node.Advertise<msgs::Double>(thrustTopic);
    gzdbg << "Publishing thrust to per-model topic: " << thrustTopic << std::endl;
  }

  // Create publisher for motor efficiency (per-model topic)
  std::string efficiencyTopic = transport::TopicUtils::AsValidTopic(
      this->dataPtr->robotNamespace + "/motor_efficiency");
  if (!efficiencyTopic.empty())
  {
    this->dataPtr->efficiencyPub = this->dataPtr->node.Advertise<msgs::Double>(efficiencyTopic);
    gzdbg << "Publishing motor efficiency to per-model topic: " << efficiencyTopic << std::endl;
  }

  // Create publisher for electrical power (per-model topic)
  std::string powerTopic = transport::TopicUtils::AsValidTopic(
      this->dataPtr->robotNamespace + "/electrical_power");
  if (!powerTopic.empty())
  {
    this->dataPtr->electricalPowerPub = this->dataPtr->node.Advertise<msgs::Double>(powerTopic);
    gzdbg << "Publishing electrical power to per-model topic: " << powerTopic << std::endl;
    gzmsg << "[MulticopterMotorModel] ✅ Created electrical_power publisher on topic: " << powerTopic << std::endl;
  }
  else
  {
    gzwarn << "[MulticopterMotorModel] ⚠️ Failed to create valid electrical_power topic from ["
           << this->dataPtr->robotNamespace << "/electrical_power]" << std::endl;
  }
    ...
    EntityComponentManager &_ecm)
{
  ...
  switch (this->motorType)
  {
    ...
    default:  // MotorType::kVelocity
    {
      ...
      // Publish thrust if publisher is valid
      if (this->thrustPub.Valid())
      {
        msgs::Double thrustMsg;
        thrustMsg.set_data(thrust);
        this->thrustPub.Publish(thrustMsg);
      }

      // Calculate motor efficiency and electrical power
      double absThrust = std::abs(thrust);
      double absOmega = std::abs(realMotorVelocity);

      // Calculate torque and shaft power
      double torque = absThrust * this->momentConstant;
      double shaftPower = torque * absOmega;

      // Calculate max thrust for normalization
      double maxThrust = this->maxRotVelocity * this->maxRotVelocity *
                         this->motorConstant;

      // Calculate normalized load and RPM factors
      double loadFactor = (maxThrust > 0.0) ? (absThrust / maxThrust) : 0.0;
      double rpmFactor = (this->maxRotVelocity > 0.0) ?
                         (absOmega / this->maxRotVelocity) : 0.0;

      // Calculate motor efficiency
      double efficiency = this->ComputeMotorEfficiency(loadFactor, rpmFactor);

      // Calculate electrical power: P_elec = P_shaft / η_motor
      double electricalPower = (efficiency > 0.0) ? (shaftPower / efficiency) : 0.0;

      // Publish motor efficiency if publisher is valid
      if (this->efficiencyPub.Valid())
      {
        msgs::Double efficiencyMsg;
        efficiencyMsg.set_data(efficiency);
        this->efficiencyPub.Publish(efficiencyMsg);
      }

      // Publish electrical power if publisher is valid
      if (this->electricalPowerPub.Valid())
      {
        msgs::Double powerMsg;
        powerMsg.set_data(electricalPower);
        this->electricalPowerPub.Publish(powerMsg);
        // Debug: log first few publications
        static int publishCount = 0;
        if (publishCount < 5)
        {
          gzdbg << "[MulticopterMotorModel] Published electrical_power: " << electricalPower
                << " W (thrust: " << absThrust << " N, efficiency: " << efficiency << ")" << std::endl;
          publishCount++;
        }
      }
      else
      {
        // Warn if publisher is invalid (only first time)
        static bool warnedInvalid = false;
        if (!warnedInvalid)
        {
          gzwarn << "[MulticopterMotorModel] ⚠️ electrical_power publisher is not valid!" << std::endl;
          warnedInvalid = true;
        }
      }
