...
#include "gz/msgs/thermal.pb.h"  // adjust based on actual path
                           ...
                           const sdf::ElementPtr &_sdf)
{
  ...
  */
  ...
  &AdvancedLiftDragPrivate::OnThermalGroup, this);
...
void AdvancedLiftDragPrivate::OnThermalGroup(const gz::msgs::ThermalGroup &_msg)
{
...
//////////////////////////////////////////////////
void AdvancedLiftDragPrivate::Update(const UpdateInfo &_info, EntityComponentManager &_ecm)
{
  ...
  gz::msgs::ThermalGroup thermalGroupCopy;
{
  ...
  {
    //std::cout << "[AdvancedLiftDrag] No thermal group available. Using ambient air.\n";
      ...
      if (std::abs(w) > 1e-3)
{
  ...
  /* std::cout << "[AdvancedLiftDrag] UAV is IN thermal ID " << thermal.id()
            << " | Distance: " << dist << " m"
            << " | Vertical wind w = " << w
            << " | Phase: " << phase
            << std::endl;  */
  ...
  break;
    ...
    {
      //std::cout << "[AdvancedLiftDrag] No UAV inside any thermal. Ambient air only.\n";
    }
  }
  ...
  if (windLinearVel != nullptr){
    ...
    cpWorld) - (windLinearVel->Data() + windVec);
  }
  else
  {
    air_velocity = worldLinVel->Data() + worldAngVel->Data().Cross(
...
GZ_ADD_PLUGIN_ALIAS(AdvancedLiftDrag,
                    "gz::sim::systems::AdvancedLiftDrag1")
